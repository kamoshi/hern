use super::lua_syntax_runtime::SYNTAX_MATCH_HELPERS;
use crate::ast::*;
use crate::syntax::{
...
fn expr_uses_syntax_runtime(expr: &Expr) -> bool {
    match &expr.kind {
        ...
        ExprKind::Call { callee, args, .. } => {
            syntax_runtime_callee(callee)
                || expr_uses_syntax_runtime(callee)
                || args.iter().any(expr_uses_syntax_runtime)
        }
        ExprKind::If {
            cond,
            then_branch,
...
fn syntax_runtime_callee(callee: &Expr) -> bool {
    matches!(
        &callee.kind,
        ExprKind::Ident(name)
            if matches!(
...
"#;
    ...
    #[test]
    fn syntax_runtime_helpers_are_not_emitted_for_plain_programs() {
        let lua = lua_for_source(
            "hern_codegen_no_syntax_runtime_test.hern",
            "let value = 1 + 2;\nprint(to_string(value));\n",
        );

        assert!(
            !lua.contains("__syntax_match")
                && !lua.contains("__syntax_equal")
                && !lua.contains("function syntax_kind"),
            "programs without syntax quotes, syntax patterns, or syntax helper calls should not emit syntax runtime helpers:\n{lua}"
        );
    }

    #[test]
    fn syntax_runtime_helpers_are_emitted_for_syntax_quote_programs() {
        let lua = lua_for_source(
            "hern_codegen_syntax_runtime_test.hern",
            "let syntax = '{foo};\nprint(syntax_kind(syntax));\n",
        );

        assert!(
            lua.contains("__syntax_template_tree")
                && lua.contains("__syntax_equal")
                && lua.contains("function syntax_kind"),
            "programs using syntax quotes/helpers should emit syntax runtime helpers:\n{lua}"
        );
    }
